...
var repositoryCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if gcs {
			remote, err = ctuf.GcsRemoteStore(ctx, repository, nil, nil)
			if err != nil {
...
func init() {
	repositoryCmd.Flags().StringVar(&repository, "repository", "repository/", "path to repository, may be GCS bucket (use --gcs true) or local file")
	repositoryCmd.Flags().BoolVar(&gcs, "gcs", false, "indicates whether the repository is a GCS bucket")
